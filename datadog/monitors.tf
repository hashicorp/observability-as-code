...
resource "datadog_monitor" "apm_service_high_avg_latency" {
  ...
  query = "avg(last_10m): ( sum:trace.rack.request.duration{service:${each.key},env:${var.environment}}.rollup(sum).fill(zero) / sum:trace.rack.request.hits{service:${each.key},env:${var.environment}}.rollup(sum).fill(zero) )"
...
resource "datadog_monitor" "apm_service_high_p90_latency" {
  ...
  query = "avg(last_10m):trace.rack.request.duration.by.service.90p{service:${each.key},env:${var.environment}} > 1"
